...
export const SolitaryDoor = ({ onComplete }: { onComplete: () => void }) => {
  ...
  const handleStart = () => {
    ...
    audioRef.current
      .play()
      .catch(() =>
        console.log("Audio trigger failed - click 'Request Access'"),
      );
    setIsUnlocked(true);
  };
  ...
  return (
    <div className="fixed inset-0 z-[1000] bg-zinc-950 flex items-center justify-center [perspective:2500px] overflow-hidden">
      ...
      <div className="absolute inset-0 -z-10 bg-zinc-950 flex flex-col items-center justify-center space-y-4">
        <Activity className="text-vhl-maroon animate-spin" size={32} />
        <span className="font-mono text-vhl-grey text-xs tracking-[0.5em] animate-pulse">
